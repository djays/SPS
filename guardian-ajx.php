<?php
        ...
        if(loginCheck() || ( isset($_COOKIE['type']) && ($_COOKIE['type'] == 'G')))                            // Main PHP Content In Here
        {    if(isset($_GET['id']))
            {
                ...
                {                
                 case 1: 
                        ...
                        $res=  mysql_query("SELECT distinct(STUDENT.REGNO),NAME FROM `STUDENT` JOIN SESS_MARKS ON STUDENT.REGNO = SESS_MARKS.REGNO AND STUDENT.REGNO IN (SELECT REGNO FROM STUDENT WHERE GID='$usn')",$con);
                ...
                case 2:
                        ...
                        echo "<div class='form-field afill-overide'><select class='autofill-combo' id='reg' >";
                        echo "<option>--Select--</option>";
                        $res=  mysql_query("SELECT distinct(STUDENT.REGNO),NAME FROM `STUDENT` JOIN END_SEM_GRADE ON STUDENT.REGNO = END_SEM_GRADE.REGNO AND STUDENT.REGNO IN (SELECT REGNO FROM STUDENT WHERE GID='$usn')" ,$con);
                        while($row = mysql_fetch_row($res))
                            echo "<option>".trim($row[0])." -- {$row[1]}</option>";
                        echo "</select></div>";        
